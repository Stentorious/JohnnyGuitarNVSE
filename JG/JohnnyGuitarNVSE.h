...
void __fastcall BipedModelUpdateWeapon(ValidBip01Names* BipedAnim, Character* fnCharacter, TESObjectWEAP* weap, int weapMods) {
	if (fnCharacter && fnCharacter->baseProcess) {
		if (auto weapInfo = fnCharacter->baseProcess->GetWeaponInfo()) {
			weapMods = ThisCall<UInt8>(0x4BD820, weapInfo); // ContChangesEntry::GetExtraWeaponModFlags
		}
	}
	ThisCall(0x4AB400, BipedAnim, weap, weapMods); // BipedAnim::UpdateWeapon
}
...
float __fastcall FixDeathSoundsTopic(HighProcess* thisObj, Actor* actor) { //Simpler fix, though we run the risk of overassumptions. 14 seconds should be more than enough though tbh.
	...
	return thisObj->dyingTimer + config::Get().iDeathSoundMAXTimer;
}
...
void UpdateMiscStatList(const char* name, int value) {
	Tile* tile = nullptr;
	auto iter = StatsMenu::Get()->miscStatIDList.list.Head();
	do
	{
...
void HandleIniOptions() {
	...
	if (settings.fixFleeing) WriteRelCall(0x8F5FE2, (UInt32)FleeFixHook);

	// for bFixItemStackCount
	if (settings.fixItemStacks) {
		WriteRelCall(0x780D11, (UInt32)DropItemHook);
		SafeWriteBuf(0x780D11 + 5, "\x90\x90\x90", 3);
	}

	// for Runtime EDIDs
	if (settings.loadEditorIDs) LoadEditorIDs();

	// for b60FPSDuringLoading
	if (settings.iFPSCapLoadScreen > 0) {
		unsigned int fpsLoadScreenPatch = floor(1000.0f / float(settings.iFPSCapLoadScreen));
	...
	if (settings.fixNPCShootingAngle) PatchMemoryNop(0x9D13B2, 8);

	// for bNoMuzzleFlashCooldown
	if (settings.noMuzzleFlashCooldown)	SafeWriteBuf(0x9BB6A8, "\x90\x90", 2);

	// for bEnableRadioSubtitles
	if (settings.enableRadioSubtitles) SafeWrite8(0x833876, 0x84);

	// fix for death topics getting cut off
	if (settings.fixDeathSounds) {
		SafeWrite16(0x8EC5C6, 0xBA90);
		SafeWrite32(0x8EC5C8, (uintptr_t)FixDeathSoundsHook);
	}
	if (settings.patchPainedPlayer) {
	...
	if (settings.removeMainMenuMusic) SafeWrite16(0x830109, 0x2574);
	...
	if (settings.bDisableDeathResponses) {
